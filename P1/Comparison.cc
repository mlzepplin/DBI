...
void OrderMaker::buildQueryOrder(const OrderMaker &sortOrder, CNF &query, OrderMaker &queryOrder, OrderMaker &cnfOrder)
{
	...
	{
		int attLocation = locateAttributeInCnf(i, query);
		if (attLocation >= 0)
		{
			...
			//Stop building queryOrder if sortorder has an attribute that is not present in cnf query
			return;
	}
}

//Locates if the attribute in sortOrder is present in the cnf query
//returns location of attribute, -1 if not found
int OrderMaker::locateAttributeInCnf(int att, CNF &query)
{
	...
	for (int j = 0; j < query.numAnds; j++)
	{
		if (query.orLens[j] == 1)
		{
			Comparison comp = query.orList[j][0];
			if (comp.op == Equals && (((comp.whichAtt1 == att) && (comp.operand2 == Literal)) || (comp.whichAtt2 == att) && (comp.operand1 == Literal)))
				return j;
		}
	}
	return -1;
